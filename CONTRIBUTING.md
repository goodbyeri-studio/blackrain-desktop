...
macOS 是当前发布验收平台。涉及 Electron main、Browser、凭据、输入、权限、安装器或更新时，在 PR 中写明 macOS 实机验证结果；没有运行的项目必须明确标注“未验证”。
...
文档是代码的一部分：产品范围看 `docs/product.md`，运行时边界看 `docs/architecture.md`，Browser 合同看 `docs/browser.md`，命令与发布边界看 `docs/development.md`，长期取舍看 `docs/adr/`。不要用目标拓扑、旧截图或测试计划代替实际验证。
