...
> 详细背景见 [CONTRIBUTING](CONTRIBUTING.md)（协作流程）、[docs/09](docs/09-运行时架构与里程碑.md)（运行时架构）。
...
> 桌面壳（CodexMonitor fork）+ DeepSeek。需要三样就位：内核二进制、网关、密钥。
...
封装了：加载 `.env` 密钥 → 把内核二进制注入 PATH → 准备 DeepSeek 的 CODEX_HOME → 起网关 → `npm run tauri dev`。按 Ctrl-C 退出会自动停网关。
...
壳默认找 PATH 上的 `codex`；CODEX_HOME 未在设置里配时，继承启动 shell 的 `CODEX_HOME` 环境变量（一键脚本已处理这两点）。config.toml 范例见 [docs/09](docs/09-运行时架构与里程碑.md)：provider 的 `base_url` 填 `http://127.0.0.1:8899/v1`、`wire_api="responses"`。
...
> `gateway/gateway.py`：responses⇄chat 翻译，纯 stdlib 零依赖。详见 [gateway/README](gateway/README.md)。
