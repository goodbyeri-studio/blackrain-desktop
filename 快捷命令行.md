...
cd apps/desktop
npm run tauri dev                               # 开发模式，热重载；首次会编 Rust 后端较久
```

壳内需把 codex 指向我们编的二进制、CODEX_HOME 指向带 DeepSeek provider 的配置（见 [docs/09](docs/09-运行时架构与里程碑.md) 的 config.toml 范例；provider 的 `base_url` 填 `http://127.0.0.1:8899/v1`、`wire_api="responses"`）。
