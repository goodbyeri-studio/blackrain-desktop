...
export function GitPerFileModeContent({
  ...
  if (groups.length === 0) {
    return <div className="diff-empty">{tx("No agent edits in this thread yet.")}</div>;
  }
            ...
            >
              ...
              <span className="per-file-group-count">
                {group.edits.length === 1
                  ? tx("{count} edit", { count: group.edits.length })
                  : tx("{count} edits", { count: group.edits.length })}
              </span>
            </button>
            {isExpanded && (
...
export function GitLogModeContent({
  ...
  return (
    <div className="git-log-list">
      {!logError && logLoading && (
        <div className="diff-viewer-loading">{tx("Loading commits...")}</div>
      )}
      {!logError &&
        !logLoading &&
        !logEntries.length &&
        !showAheadSection &&
        !showBehindSection && <div className="diff-empty">{tx("No commits yet.")}</div>}
      {showAheadSection && (
        <div className="git-log-section">
          <div className="git-log-section-title">{tx("To push")}</div>
      ...
      {showBehindSection && (
        <div className="git-log-section">
          <div className="git-log-section-title">{tx("To pull")}</div>
          <div className="git-log-section-list">
      ...
      {(logEntries.length > 0 || logLoading) && (
        <div className="git-log-section">
          <div className="git-log-section-title">{tx("Recent commits")}</div>
          <div className="git-log-section-list">
  ...
  return (
    <div className="git-issues-list">
      {!issuesError && !issuesLoading && !issues.length && (
        <div className="diff-empty">{tx("No open issues.")}</div>
      )}
      {issues.map((issue) => {
  ...
  return (
    <div className="git-pr-list">
      {!pullRequestsError && !pullRequestsLoading && !pullRequests.length && (
        <div className="diff-empty">{tx("No open pull requests.")}</div>
      )}
        ...
        return (
          ...
          >
            ...
            <div className="git-pr-meta">
              ...
              {pullRequest.isDraft && (
                <span className="git-pr-pill git-pr-draft">{tx("Draft")}</span>
              )}
            </div>
          </div>
        );
