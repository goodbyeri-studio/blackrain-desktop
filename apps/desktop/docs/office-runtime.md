...
The current implementation proves that repository resources and a shared codex preparation path exist; it does not prove current Windows installer inclusion, installed-runtime behavior, an installable workbench, or activated task execution. There is no active Office/workbench spec; recovery of this route requires a product-priority change and a new sole spec.
