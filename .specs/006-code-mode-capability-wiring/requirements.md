...
> **事实状态(2026-07-31)**:42 个方法的 5 层包装代码与编译/shape 记录已存在,但完整 capability 验证基线最新只到 `cfead68`;当前文档锁定 rust-v0.146.0 / `e363b08c` 只完成源码与官方 Windows package 供应链锁定,尚未重跑能力探针或完成 Windows GUI 冒烟。「包装代码存在」、「当前内核验证通过」、「GUI 可用」是三个不同状态。
...
- 相关上游/文档:本 spec 实装/shape 探针基线为 `bdd282f`→`cfead68`;当前锁定已更新到 rust-v0.146.0 / `e363b08c`,下一轮 CODE GUI 开发前必须重跑探针并刷新缺口表;运行时边界见 [docs/09](../../docs/09-运行时架构与里程碑.md)。
...
- 壳完整参数用法有 `cfead68` 的历史字段级兼容记录;当前锁定 `e363b08c` 必须重跑能力 shape 探针,并在 Windows 上跑目标 Electron GUI 冒烟。重验前不得把 42 个方法宣称为当前全量可用。
