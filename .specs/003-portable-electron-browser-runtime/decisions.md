...
### 2026-08-21：采用 MIT 作为公开源码底座的默认许可证

- 决策：BlackRain 自有代码、Browser Runtime 自有代码和随包 Browser client 默认按仓库根目录 MIT License 发布；CodexMonitor、OpenAI Codex 和其他第三方内容继续按各自许可证保留。
- 原因：MIT 与现有 Electron 壳的上游归属兼容，便于 Electron Agent 客户端复制、fork 和二次集成；Apache-2.0 Codex runtime 仍保持独立黑盒和独立 NOTICE。
- 影响：对外文档、组件 manifest、NOTICE 和发布审计必须明确代码所有权，不得把第三方 runtime 重新授权为 MIT。
- 发布边界：许可证落地不等于 GitHub 仓库已经公开；切换可见性前仍须完成 Git 历史、私有资料、第三方资产和密钥审计。

## 待决策

- 首个 `PORTABILITY_PASS` 后是否建立独立 workspace package 或移动到顶层 `packages/`。
- Electron 42.3.0 之外的最低支持版本和兼容矩阵。
- Browser Runtime 是否在首轮公开后拆分为独立 package、版本策略和分发渠道。
