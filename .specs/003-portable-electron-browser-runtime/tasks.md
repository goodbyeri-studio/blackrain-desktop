...
- [ ] `B6-05` 更新 README、04、09、commands、第三方 License/NOTICE 审计和对应 spec；在 MIT 公开基线下仍不得把源码底座写成已通过 `PORTABILITY_PASS` 的 SDK。
- [ ] `B6-06` 清理失效兼容代码、重复事件出口、额外 constructor/composition entry 和悬空文档，重跑静态扫描与 runtime identity test。

## 对外分发门禁（不计入 `PORTABILITY_PASS`）

- [ ] 审计 Git 历史中的密钥、私有资料、未授权二进制和第三方来源，并决定是否需要重写历史或使用公开镜像。
- [ ] 按根 `LICENSE`、`NOTICE` 和每个 runtime manifest 重建分发文件清单。
- [ ] 公开仓库设置、分支保护、Private Vulnerability Reporting、CI required checks 和首版发布矩阵完成。
