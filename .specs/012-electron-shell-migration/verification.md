...
| 2026-07-30 | CI 门禁与首次 `windows-latest` 实跑 | GitHub Actions run `30525783917` + 本地隔离回归 | 部分通过 | Windows Rust、JS、Gateway PASS；Electron package/smoke PASS，Playwright E2E 启动失败并跳过 MSIX make。已让 smoke/E2E 等待进程退出并使用独立测试 app-data，本地连续 smoke/E2E PASS，待远端复验 |
