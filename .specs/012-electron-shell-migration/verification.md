...
| 2026-07-30 | CI 门禁与 `windows-latest` 实跑 | GitHub Actions runs `30525783917` / `30528857360` / `30529234179` / `30529634781` / `30529985702` / `30530578031` + 本地隔离回归 | 部分通过 | Windows Rust、JS、Gateway 已获 PASS；显式 runtime、窗口固定及 DOM click 后 Browser 侧栏已在 CI 挂载，随后无交互桌面的 `page.screenshot()` 超时并跳过 MSIX make。截图不参与断言，现仅在本地生成；远端继续验证 host/UI 合同、受限 Browser 控制和 MSIX，待复验 |
