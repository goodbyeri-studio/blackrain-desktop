...
| 2026-07-30 | CI 门禁与 `windows-latest` 实跑 | GitHub Actions runs `30525783917` / `30528857360` / `30529234179` / `30529634781` / `30529985702` + 本地隔离回归 | 部分通过 | Windows Rust、JS、Gateway 已获 PASS；显式 Electron runtime 安装后 Playwright 已成功连接 inspector/CDP，但 CI 虚拟桌面中 Browser 侧栏的 role-visible 等待超时，MSIX make 被跳过。已固定/显示主窗口，改用 DOM 原生 click 和稳定 test id 挂载等待，并保留规范化 entry、进程退出等待和独立测试 app-data，待远端复验 |
