...
| 2026-07-30 | CI 门禁与 `windows-latest` 实跑 | GitHub Actions runs `30525783917` / `30528857360` / `30529234179` / `30529634781` + 本地隔离回归 | 部分通过 | Windows Rust、JS、Gateway 已获 PASS；Electron package/smoke PASS，但 Electron 42 不再声明 npm install script，`npm ci/rebuild` 均未生成开发态 runtime，Playwright E2E 因而失败并跳过 MSIX make。已显式接入依赖自带的 `install.js`，并保留规范化 entry、进程退出等待和独立测试 app-data，待远端复验 |
