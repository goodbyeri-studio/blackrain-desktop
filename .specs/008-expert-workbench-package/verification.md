...
| 2026-07-12 | activation generation → MCP router swap | migration target 重新读取 verified activation/plugin runtime；credential 只解析到内存 desired；router connect-before-swap 后才允许 runtime readiness/commit；swap 失败回滚 binding 且旧 generation 继续；commit 失败从旧 binding 重建 router generation | Python router E2E；`cargo test hermes --lib`; `cargo test workbench_core --lib`; `cargo test workbench::tests --lib`; `cargo check`; `git diff --check` | Python `8 passed`；Hermes `116 passed`；workbench Core `21 passed`；adapter `2 passed`；check/diff 通过（macOS） | 代码与协议级补偿已接通；锁定 Hermes next-turn、真实 migration fault/App 强退和 Windows process tree 未验证，不把该行当完整升级验收 |
...
- generation migration 的代码级状态机、持久审计、动态 MCP router 和代码级补偿恢复已实现；锁定 Hermes next-turn、真实故障/App 强退和 Windows 验证未完成。
