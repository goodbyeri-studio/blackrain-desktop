...
> 状态校准（2026-07-12）：M-A1 账号代码、M-A2 过渡代理和真实 Supabase/DeepSeek 计量已经验证；生产项目边界已按 010 定为 Cloud 购买 MeiMei API 服务，但 broker、对账、Windows 桌面 GUI、WORK/Hermes credit 和 Plus BYOK 尚未实现。2026-06-25 的公网地址和云服务结果是当日历史证据，不代表当前可用性监控。
...
| YYYY-MM-DD | WORK/Hermes credit | Windows 桌面 GUI | 未跑 | Cloud broker + MeiMei API token/usage 对账尚未实现 |
...
- WORK/Hermes credit、Plus BYOK、Cloud broker 和 MeiMei API 对账尚未实现，不能由现有 CODE 过渡代理证据外推。
