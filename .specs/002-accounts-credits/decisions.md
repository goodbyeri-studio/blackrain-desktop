...
- 替代方案：不开确认（注册即登录，曾短暂采用）；纯用户名合成邮箱；短信验证码；magic link 确认。
- 为什么不用替代方案：不开确认无法验真邮箱（找回/触达无保障）；纯用户名增加唯一性/校验复杂度；短信需企业实名+签名报备，重；magic link 桌面无深链。
- 影响范围：`AccountAuthCard`（两步状态机 + 验证码字段 + 重发/返回）、`accountService`（`verifySignupOtp`/`resendSignupOtp`）、`AccountProvider`（契约扩展）、`supabase/config.toml`（开确认 + SMTP + 模板）、`supabase/templates/confirmation.html`（验证码邮件）。
- 后续复查条件：① Resend 国内送达率不达标 → 迁国内发信；② 要做密码找回 → 复用同套 SMTP + recovery OTP。
...
- 决策：`blackrain-cloud` 是 `blackrain-relay` 的企业客户。Supabase JWT 只向 Cloud 证明身份；Cloud 检查套餐/余额后向 Relay 兑换长期、可撤销、可限额的 model token。所有 codex 会话经 Desktop 本地翻译网关进入 Relay。现有 `gateway/proxy.py` 不是目标生产入口。
- 原因：Relay 要独立经营并服务第三方，Cloud 不应与其共享数据库或进入高吞吐模型内容路径。
