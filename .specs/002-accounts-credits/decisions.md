...
## 2026-06-25：注册用邮箱+密码（不开邮箱确认）+ 注册时二次确认密码

- 决策：账号标识保留邮箱（Supabase 原生），注册不开邮箱确认（`enable_confirmations=false`）→ 注册即自动登录、不发任何邮件；注册表单加「确认密码」字段，前端校验两次一致才提交。
- 原因：① Supabase 无原生用户名，纯用户名需合成邮箱绕路且要自管唯一性/校验，收益不抵成本；② 不开确认就绕过了「境外 SMTP 发到 QQ/163 进垃圾箱」的送达坑；③ 二次确认是注册防误输的标准 UX。
- 备案澄清：ICP 备案只约束「域名+大陆境内服务器」。Supabase 在境外、代理在 DO 新加坡、本体是桌面 App，均不触发备案；改 auth 方式也不影响备案与否。真正相关的是 PIPL 跨境数据合规（待法务确认），与本决策正交。
- 替代方案：纯用户名（合成邮箱映射）；短信验证码登录。
- 为什么不用替代方案：纯用户名增加唯一性/校验与未来找回的复杂度；短信需企业实名+签名报备+短信服务商，重。
- 影响范围：仅 `AccountAuthCard`（前端校验 + 确认密码字段 + 注册成功文案）；`accountService.signUp/signIn` 与数据库 trigger 不变。
- 后续复查条件：若要做密码找回，需引入可用 SMTP（自配发信域名）或改短信；届时重审「不开邮箱确认」。
