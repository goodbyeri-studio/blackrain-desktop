...
- 已验证过渡代理是「最小 OpenAI 兼容转发 + 计量」；生产接缝改为 Supabase JWT 只向 Cloud 证明身份，Cloud 向 Relay 兑换长期、可撤销、可限额的 model token。Desktop 不把 Supabase JWT 直接当 Relay 常驻凭据。
...
- [ ] Desktop/Gateway 如何从 Cloud broker 获得、刷新和撤销 Relay token，并为两种 surface 统一商业余额和错误语义。
