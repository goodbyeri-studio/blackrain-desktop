...
> **状态（2026-07-26）：暂停。** 下文是冻结候选设计，不进入当前 Electron/Codex-first 主线。
...
- 属于 `gateway` 的逻辑：无。插件工具调用不直接经过网关；承载插件任务的 codex 模型会话仍统一经过网关。
...
用户安装/激活工作台，或在任务中临时启用插件
  ...
  -> Session Orchestrator 按受控会话描述符注册工具
  -> 当前 surface 通过共享 codex thread 调用插件 tool
...
- 模型/网关失败：会话进入可重试失败态；插件进程和本地成果不得被误报为执行成功。
...
- 协议探针：MCP server 起停 + `tools/list_changed` 动态发现；真实会话接入与恢复按 spec 011 验证。
