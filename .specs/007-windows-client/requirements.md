...
- [x] **CI 是否存在**:已有 `.github/workflows/ci.yml`(Ubuntu 跑 JS typecheck/test,Windows 跑 Rust `cargo check`)。它不打 NSIS,也不是完整 Windows 发布矩阵;是否扩充为 Windows typecheck/test/build 仍待决。
