...
- [x] **CI 是否存在**:已有 `.github/workflows/ci.yml`；Ubuntu 跑 JS typecheck/test，Windows runner 已配置 JS typecheck/test/lint/DS/codemod + Rust `cargo check`/Hermes/workbench/plugin 专项。它不 vendor/启动 Hermes、不打 NSIS，也不是完整 Windows 发布矩阵。
