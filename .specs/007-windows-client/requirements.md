...
- [x] **CI 是否存在**:已有 `.github/workflows/ci.yml`；PR 先按路径分流，Ubuntu 承担前端 typecheck/test/lint/DS/codemod，只有 Rust/WORK 相关变化才启用 Windows 并跑 Hermes/workbench/plugin 专项。它不 vendor/启动 Hermes、不打 NSIS，也不是完整 Windows 发布矩阵。
