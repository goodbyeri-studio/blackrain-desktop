...
| 2026-07-12 | 工作台停用纵切 | activation 串行锁/冲突门禁、graceful stop 尝试、supervisor stop、stream cancel、task cancelled 收敛、Skills/MCP unbind、activation 最后删除、用户项目保留；Tauri/TS/controller/DS confirmation | `cargo test workbench_core --lib`; `cargo test hermes --lib`; `cargo check`; `npm run typecheck`; targeted Vitest；`npm run test`; `npm run lint`; `npm run lint:ds`; `npm run codemod:ds:dry` | Rust `8 + 83 passed` + check；targeted TS `82 passed`；全量 `149 files / 1092 tests` + typecheck；lint/DS 0 error、5 条既有 warning（macOS） | Store 单测证明项目文件未被删除；codemod 仅提示既有 Settings modal；没有真实 Windows Hermes/MCP process-tree、Tauri WebView 或 installer lifecycle 实测；008 activate producer 仍不存在 |
...
| 2026-07-12 | Office bundled Manifest 接缝 | 008 strict manifest inspect、official allowlist App command、唯一 TS wrapper、controller bootstrap、无 activation 时真实包详情/依赖计数展示 | `cargo test workbench_core --lib`; `cargo check`; targeted Vitest；`npm run test`; `npm run typecheck`; `npm run lint`; `npm run lint:ds`; `npm run codemod:ds:dry`; `git diff --check` | Rust `13 passed` + check；targeted TS `4 files / 91 passed`；全量 `149 files / 1106 tests` + typecheck；lint/DS 0 error、5 条既有 warning（macOS/jsdom） | 只读 inspect，不是 install/health/permission/activate；未生成 activation，WORK Composer 仍正确禁用；Daemon/Windows 未验证 |
...
- activation store、Skills/MCP/environment/session 的代码级消费门禁已实现，但 008 Manifest/install/verify/activate pipeline 尚未生成正式 activation；当前无可发布的 Office 激活实例。
