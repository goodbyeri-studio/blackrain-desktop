...
## 2026-07-12：首版关闭 Hermes Cron/MoA，PTY 只复用 BlackRain task-scoped terminal，自验证归 008

- 决策：managed Hermes config 在 `agent.disabled_toolsets` 中禁用 `cronjob`；首版不嵌入 Hermes Desktop/TUI PTY，但允许 WORK resource rail 复用 BlackRain 现有 `portable_pty + xterm`。前端只传 `taskId`，Core 从 TaskStore 解析并校验项目根，使用隔离的 `work:<taskId>` session scope，显式启动/停止，切换 rail 只 detach。它是用户终端，不冒充 Hermes tool 内部进程。不生成 MoA preset 或选择器。自验证继续服从 008 的包验证、环境验证、领域验证三层合同；运行时只有来自 verified activation 的工具/断言可以产出验证结果，模型文字自评不能把任务或工作台标成已验证。
...
- 替代方案：直接开放 `cronjob` 工具、复用 Hermes gateway scheduler、嵌入上游 PTY、允许前端传任意 cwd、把 Agent tool 输出伪装成可 attach shell、把 MoA 当高阶套餐默认打开、或让 agent 在最终回答中声称“已验证”。这些方案分别引入生命周期旁路、目录逃逸、虚假能力、Windows 运行时分叉、不可控多倍计费和伪验证。
...
- 后续复查条件：定时任务需另建工作室级 spec，至少包含 schedule owner、activation pin、权限/审批、模型与预算、离线/休眠、幂等、输出和审计；PTY 仍需 Windows `powershell.exe`/`pwsh.exe`、resize、stop、任务切换和 process-tree 实测；MoA 需 broker 返回允许模型并建立多调用 credit/质量基线。008 领域验证仍必须在 Office 黄金流程中落真实输出断言。

## 2026-07-12：不直接暴露 Hermes native session fork

- 决策：当前 WORK sidebar 不调用 `POST /api/sessions/{session_id}/fork`。rename、pin、archive/restore 由 BlackRain TaskStore 管理；fork 等 BlackRain 冻结 parent/child task lineage、父任务继续策略和本地 normalized journal 克隆事务后再接。
- 原因：锁定 Hermes fork 会把源 session 以 `branched` 结束，并创建携带 transcript 的子 session；而当前 BlackRain task/session 是永久一对一，continue 又会从本地 journal 构造显式 history。直接增加按钮会让父 task 继续时可能解析到子 session，或让两个 task 共享/分歧于同一 lineage，破坏零串台铁律。
- 替代方案：只在前端复制任务卡、只克隆 BlackRain journal 不调用 Hermes、调用 fork 后让父 task 继续可用、或静默把父 task 的 session id 改成子 id。这些方案都会伪造 fork、丢失上游 lineage 或制造跨 task 会话混用。
- 影响范围：WORK sidebar/session picker、TaskStore schema、Hermes client session API、continue/recovery 和删除/归档语义。
- 后续复查条件：另行冻结 `parentTaskId`/branched 状态、父任务是否只读、fork point、事件 ID 重写、上游 fork 成功而本地失败的补偿、删除父/子保留策略和 Windows 恢复矩阵后，才能把 native fork 计入迁移完成面。
