...
- 相关上游/文档/现有实现：
  - codex-upstream（内核黑盒，app-server JSON-RPC，当前锁定 `da4c8ca`；旧底账基线 `51b3cd5`/`bdd282f`，待重核）
  - Hermes Agent: <https://github.com/NousResearch/hermes-agent>（MIT，Python agent loop，当前锁定 v2026.7.1 / `7c1a029`；旧底账基线 `a6a28ce`，待重核）
