...
4. 用真实验证证据区分代码存在、运行通过和 macOS 产品发布就绪。

## 变更流程

- 小型 bug 修复和文档改动走普通 PR。
- 产品范围、公共 Browser contract、许可证、数据边界或发布平台变化，必须先更新 `docs/product.md`、`docs/architecture.md`、`docs/browser.md` 或 `docs/adr/`。
- `main` 只接受通过 review 和 CI 的 PR，使用 Squash merge。
- 维护者可以拒绝超出项目范围、违反许可证或无法验证安全边界的改动。

## 版本与发布

版本、锁定的 Codex runtime、macOS 制品和发布说明必须来自可复现的 CI/本地命令。正式签名、公证、安装、升级、回滚和卸载矩阵通过前，不发布“稳定版”或作出产品可用承诺。
