...
普通 PR runner 不持有证书。GitHub `Electron Windows Release` workflow 使用 `windows-release` Environment、人工审批、vars/secrets 和临时 PFX 文件；完成后删除签名材料。artifact 仍必须经过下面的产品矩阵才能发布。
