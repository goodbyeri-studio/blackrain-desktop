...
# managed config → /health/capabilities → /v1/runs → SSE → completed
...
`test-hermes-live-probe.py` 不读取 `.env` 或用户凭据，使用随机 loopback 端口和固定测试文本，并检查 `memory`、`session_search`、`cronjob` 未进入模型工具列表。它证明锁定 Hermes 真进程可消费 BlackRain managed config 并完成结构化 run/SSE，不证明真实 new-api、国产模型、Tauri 或 Windows 产品闭环。
